...
- Evidence anchor validity requirement (CKSPEC-ENF-011) — conformance tooling MUST validate that file-path anchors exist on disk and that file::symbol anchors contain the named symbol; a dangling anchor MUST cause the generator to fail. Discovered through ckeletin-rust review (PR #32): a renamed shadow-log test left a stale anchor in the mapping that passed the ENF-008 anchor-presence check silently. Both implementations deferred pending adoption. See ckeletin-rust issues #15 and PR #32.
